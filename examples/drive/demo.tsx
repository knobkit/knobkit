import { knobkit, tree, breadcrumb, output, button, upload, menu, grow, row, col, drawer, mediaUrl } from "knobkit";
...
app.on(uploader.changed, async (files) => {
  for (const f of files) {
    const id = `n${nextId++}`;
    const body = f.type.startsWith("image/") ? `![${f.name}](${mediaUrl(f.ref)})` : `**${f.name}**`;
