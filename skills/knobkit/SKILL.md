...
a signature is still unclear, the source of truth is the widget def at
`node_modules/knobkit/src/widgets/<name>/def.ts` (the package ships its source).
Inside the knobkit monorepo, read `packages/knobkit/src/widgets/<name>/def.ts` instead.
</core_principle>
...
ONE closest to the request and adapt it; copy it wholesale to start a new project.
  ...
  <project name="caption-serve" tier="serve" use="upload an image; a server-side model processes it → output; shows MediaRef → bytes decoding" />
  ...
  <project name="webcam-live" tier="mount" use="continuous webcam frames → per-frame processing → image; shows the latest-policy channel handling for live streams" />
</reference_projects>
...
**either way** — the server holds no app data, so restarts and reconnects are free. If unsure, pick
mount (faster loop, and one line moves it to serve later) and say so.
...
`knobkit/tsconfig.base.json`, and the entry ends in `mount("#root")` or `serve()`. Node ≥ 22.
...
2. **Compose** — `knobkit({ title, description, widgets })`. `widgets` is a tree: a single widget, an
   array (implicit `col`), or `row`/`col`/`grid`/`tabs`/`splitPane`/`accordion` nesting. Add
   `fill: true` for a full-bleed split-pane/dashboard layout.
...
(inferred type — never annotate a pipeline, the union is too complex) or in `app.setup` bracketed with
`busyStart()`/`busyEnd()`, streaming via `say` + `append`, and binary data as **MediaRefs** — an
uploaded image or webcam frame arrives as a `MediaRef`; decode with `await mediaBytes(ref)` or load
via `mediaUrl(ref)`, never expect raw bytes or data URLs in state.
...
- **Reads are async** — `const v = await box.value();`, never `box.value()` without `await`. Same for
  `history()`, `data()`, `all()`. (A read observes every write issued before it, so write-then-read
  is safe within a handler.)
- **Widget methods only run inside a handler or `setup`** — declaring widgets at module scope is fine;
  *calling their methods* there is not.
- **Value inputs (`text`, `number`, `dropdown`, `code`) emit only `changed`**, and the payload IS the
  value (no `.submitted`). Drive one-shot actions from a `button`'s `.clicked` + `await input.value()`.
  `upload` is different: it emits `picked` with a **MediaRef**, and `await upload.value()` returns a
  `MediaRef | null`.
- **Binary data is a MediaRef, never bytes in state** — decode uploads with
  `new Blob([await mediaBytes(ref)], { type: ref.mime })`, display with `mediaUrl(ref)` or
  `image.show(ref)`.
- **`mount("#root")` needs the selector**; `serve()` takes none.
- **Live streams need no drop guard** — mic `clip` and webcam `frame` are latest-policy channels:
  stale frames are dropped automatically while the handler runs. Don't hand-roll a `processing` flag.
...
- NEVER invent widget types, factory options, or methods. The catalog is in `references/api.md`; the
  source of truth is the def at `packages/knobkit/src/widgets/<name>/def.ts` in the knobkit repo, or
  `node_modules/knobkit/src/widgets/<name>/def.ts` inside a generated app. Verify before relying on a
  signature.
...
- Examples must use only the public authored API (`knobkit`, widget factories, `on`, widget methods,
  `mount`/`serve`, the media helpers). Reaching for an internal means the approach is wrong.
...
- [ ] Handlers use async reads, structured-edit writes, MediaRefs for binary data, and `busy` where apt
- [ ] No fabricated widgets/methods; uncertain signatures verified against the defs
