import { knobkit, upload, button, output, col, mediaBytes } from "knobkit";
...
app.on(
  ...
  go.busy(async () => {
    ...
    // an upload is a MediaRef; pull the bytes on demand — they never live in state
    const img = await RawImage.fromBlob(new Blob([await mediaBytes(file)], { type: file.mime }));
