...
// A live webcam filter: the camera emits a frame (as a MediaRef) every `every` ms; each frame is
// inverted on a canvas and shown. `frame` is a latest-policy channel — frames arriving while the
// handler is still working are dropped automatically, so no hand-rolled guard is needed.
